...
/*   Updated: 2018/04/30 17:41:36 by tmervin          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	mandel_plot(t_env *e)
{
	...
	while (x < WIN_WIDTH + e->x)
	{
		...
		{
			e->mandel->cr = (double)x / e->mandel->zoom + e->mandel->x1;
			e->mandel->ci = (double)y / e->mandel->zoom + e->mandel->y1;
			e->mandel->zr = 0;
			e->mandel->zi = 0;
			e->imgstr[x + y * WIN_WIDTH] = get_color(e, mandel_rang(e));
			y++;
		}
		x++;
	}
